...
static EBPF_INLINE void maybe_add_tlcr_info(Trace *trace)
{
  ...
  if (bpf_probe_read_user(&record_ptr, sizeof(record_ptr), (void *)tlcr_ptr_addr)) {
    ...
    DEBUG_PRINT("[TLCR] tid=%d ERR read at 0x%llx", trace->tid, tlcr_ptr_addr);
    return;
  }

  DEBUG_PRINT("[TLCR] tid=%d addr=0x%llx ptr=0x%llx",
              trace->tid, tlcr_ptr_addr, (u64)record_ptr);
  ...
  if (bpf_probe_read_user(&record, sizeof(record), record_ptr)) {
    ...
    DEBUG_PRINT("[TLCR] tid=%d ERR record read at 0x%llx", trace->tid, (u64)record_ptr);
    return;
  }
  if (record.valid != 1) {
    DEBUG_PRINT("[TLCR] tid=%d valid=%d, skipping", trace->tid, record.valid);
    return;
  }
  ...
  DEBUG_PRINT("[TLCR] tid=%d OK span=%016llX", trace->tid,
              trace->apm_transaction_id.as_int);
}
