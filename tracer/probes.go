...
// from consuming kernel memory in the main collection. A custom probe owns
// ELF resources prefixed by its configured name plus "_"; Probe.Load copies
// those resources into an isolated collection when the probe is enabled.
func removeDynamicProbeResources(collection *cebpf.CollectionSpec, prefixes []string) {
	...
	for name := range collection.Variables {
		if matches(name) {
...
type ProbeContext struct {
	...
	registerMetrics func(func() []metrics.Metric) (MetricsSubscription, error)
}

// MetricsSubscription owns a dynamic probe's metric callback and preserves
// its final counter deltas when the probe closes.
