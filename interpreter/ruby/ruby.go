...
func (r *rubyData) Attach(ebpf interpreter.EbpfHandler, pid libpf.PID, bias libpf.Address,
	...
	cdata := support.RubyProcInfo{
		...
		// Skip native resume for cfuncs to save tail calls.
		// Opt-in via OTEL_EBPF_RUBY_SKIP_NATIVE_RESUME=true (default: false).
		Skip_native_resume: skipNativeResume(),
	}

	if cdata.Skip_native_resume {
		log.Infof("Ruby: skip_native_resume enabled for pid %d, cfuncs will be pushed inline", pid)
	}
