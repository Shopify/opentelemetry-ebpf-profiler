...
func Loader(_ interpreter.EbpfHandler, info *interpreter.LoaderInfo) (
	...
	if pclnData == nil {
		return nil, fmt.Errorf("failed to identify .gopclntab")
	}
	...
	if runtimeTextAddr == 0 {
		return nil, fmt.Errorf("failed to get address of runtime.text")
	}

	pcln := gosym.NewLineTable(pclnData, runtimeTextAddr)
	if pcln == nil {
		return nil, fmt.Errorf("failed to create Line Table from .gopclntab")
	}
