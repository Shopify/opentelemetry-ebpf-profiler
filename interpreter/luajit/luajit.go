...
func extractInterpreterBounds(intervals *sdtypes.IntervalData, param int32,
	...
	for _, block := range intervals.Blocks {
		for i := range block.Deltas {
			...
			start := block.Start + uint64(delta.Offset)
			end := block.End
			if i+1 < len(block.Deltas) {
				end = block.Start + uint64(block.Deltas[i+1].Offset)
			}
			...
			// The first case covers x86 with DWARF and old versions of LuaJIT ARM that
			// used DWARF. The second covers more recent ARM versions that use frame
			// pointers.
			if delta.Info.BaseReg == support.UnwindRegSp && delta.Info.Param == param ||
				delta.Info.BaseReg == support.UnwindRegFp && delta.Info.Param == 16 {
...
// UsesAnonymousMappings reports that LuaJIT needs the process manager to sync
// anonymous executable mappings. LuaJIT emits JIT traces (mcode) into anonymous
// executable mmap regions; without these mappings synced into
// pid_page_to_mapping_info, the native unwinder treats an mcode PC as an
// unsupported anonymous mapping (ERR_NATIVE_UNSUPPORTED_MAPPING) instead of
// handing off to the LuaJIT unwinder. (Interface added upstream by #1473,
// after parca #1236.)
