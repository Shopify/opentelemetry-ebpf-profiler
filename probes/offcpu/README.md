...
`offcpu` measures **how long** each stack was off CPU, not how often it left
the CPU. It follows the classic `offcputime` model (Gregg, *BPF Performance
Tools*, Ch06): the departure time of every thread is recorded as it is
...
- `min_duration` and `sample_rate` are applied at **switch-in**, on the
  ...
  off-CPU profiling which flips a coin at switch-out. `min_duration` is the
  primary volume control — with the default of 0 every wakeup on the system
  that survives sampling is unwound. Start with milliseconds, not zero.
...
|---|---|---|
| decision point | random sample at switch-out | filter at switch-in on measured duration |
| long waits | missed with probability 1-p | always measured |
...
custom_probes:
  off_cpu_time:
    min_duration: 1ms   # emit only waits at least this long (default 0)
