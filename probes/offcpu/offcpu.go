...
const (
	...
	// A non-zero default prevents an empty configuration from unwinding every
	// scheduler wakeup on the host. Users may explicitly set 0s for exhaustive
	// capture after accepting the overhead.
	defaultMinDuration = time.Millisecond
...
// reaches the threshold are eligible for unwinding. sample_rate then applies
// to qualifying wakeups, not to switch-outs.
func New(rawConfig any) (tracer.Probe, error) {
	rawConfig = configWithDefaultMinDuration(rawConfig)
...
func configWithDefaultMinDuration(rawConfig any) any {
	if rawConfig == nil {
		return map[string]any{"min_duration": defaultMinDuration}
	}
	config, ok := rawConfig.(map[string]any)
	if !ok {
		return rawConfig
	}
	if _, configured := config["min_duration"]; configured {
		return rawConfig
	}
	withDefault := make(map[string]any, len(config)+1)
	for key, value := range config {
		withDefault[key] = value
	}
	withDefault["min_duration"] = defaultMinDuration
	return withDefault
}
...
func (p *offCPUTimeProbe) Load(origin uint16, ctx *tracer.ProbeContext) (link.Link, error) {
	...
	resources.Add(&finalMetricsCloser{subscription: subscription, collect: collector.Collect})
...
func attachSwitchIn(program *cebpf.Program, resources *probeutil.Resources) error {
	...
	if len(errs) != 0 {
		log.Warnf("Off-CPU time attached to %d of %d %s symbols; partial failures: %v",
			attached, len(symbols), switchInSymbolPrefix, errors.Join(errs...))
	}
	return nil
}
...
func (p *offCPUTimeProbe) LabelTrace(trace *libpf.EbpfTrace) {
	if trace.ProbeUserData == stateUnavailable {
		return
	}
	...
	trace.CustomLabels[libpf.Intern(threadStateLabelKey)] =
		libpf.Intern(threadStateName(trace.ProbeUserData))
}
